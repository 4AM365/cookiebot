...
// `@COOKIEBOT v1@ {json}` line at the bottom; pasting the whole thing back
// reloads every input exactly. Both helpers are plain DOM/JSON, no deps.
const IO_TAG = "COOKIEBOT";
...
export default function CookieBuildSheet() {
  ...
  const recipeText = useMemo(() => {
    const L = [];
    L.push(`CookieBot — ${recipeName}`);
    L.push(`${special ? "Fixed recipe" : boundStyle ? "Bound style (sliders tune within it)" : "Freestyle"} · ${f} g flour base${specialRecipe ? "" : ` · makes ~${cookieCount} ${sc.label.toLowerCase()} cookies`}`);
    ...
    groups.forEach((g) => {
      ...
      g.items.forEach((it) => {
        const amt = it.g != null ? `${it.g} g${it.pct != null ? ` (${it.pct}%)` : ""}` : "to taste";
        L.push(`    ${it.k} … ${amt}`);
      });
    });
    L.push("");
  ...
  function loadRecipe() {
    ...
    if (!d) { setIoMsg("⚠ No recipe data found — paste a full CookieBot export (incl. the @COOKIEBOT@ line)."); return; }
    if (d.kind && d.kind !== "cookie") { setIoMsg(`⚠ That's a ${d.kind} recipe, not a cookie one.`); return; }
    try {
      if (typeof d.flourG === "number") setFlourG(d.flourG);
      if (d.q && typeof d.q === "object") setQ(d.q);
      if (d.scoop) setScoop(d.scoop);
  ...
  function downloadRecipe() {
    downloadTextFile(`${slugify(recipeName)}-cookies.txt`, recipeText);
    ...
    <div className={geocities ? `geocities ${dark ? "geo-dark" : "geo-light"}` : undefined} style={{ background: C.paper, minHeight: "100vh", padding: "28px 16px 60px", fontFamily: "'Fraunces', serif", color: C.ink, colorScheme: dark ? "dark" : "light", backgroundImage: C.glow, transition: "background .25s ease, color .25s ease" }}>
      ...
      <div style={{ width: "100%", maxWidth: 880, margin: "0 auto", animation: "riseIn .5s ease" }}>
        ...
        <div style={{ marginTop: 26, border: `1.5px solid ${C.line}`, borderRadius: 14, background: C.card, overflow: "hidden" }}>
          <button onClick={() => setIoOpen((o) => !o)} style={{ width: "100%", display: "flex", justifyContent: "space-between", alignItems: "center", gap: 10, background: "transparent", border: "none", cursor: "pointer", padding: "14px 16px", fontFamily: mono, fontSize: 11, letterSpacing: 2, textTransform: "uppercase", color: C.choc, fontWeight: 600 }}>
            <span>Save / load this recipe</span>
            <span style={{ fontSize: 20, color: C.butter, transform: ioOpen ? "rotate(45deg)" : "none", transition: "transform .2s ease", lineHeight: 1 }}>+</span>
          </button>
          {ioOpen && (
            <div style={{ padding: "0 16px 18px", animation: "riseIn .25s ease" }}>
              ...
              <label style={{ display: "block", fontFamily: mono, fontSize: 10.5, letterSpacing: 1, textTransform: "uppercase", color: C.inkSoft, fontWeight: 600, marginBottom: 5 }}>Notes — how you liked this batch</label>
              <textarea value={notes} onChange={(e) => { setNotes(e.target.value); setIoMsg(""); }} rows={3} placeholder="e.g. baked 1 min longer for more snap; doubled the flaky salt — perfect."
              ...
              <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginTop: 8, marginBottom: 16 }}>
                <button onClick={copyRecipe} style={{ border: "none", borderRadius: 9, padding: "9px 16px", cursor: "pointer", fontFamily: mono, fontSize: 12, fontWeight: 600, background: C.butter, color: C.onAccent }}>Copy</button>
                <button onClick={downloadRecipe} style={{ border: `1.5px solid ${C.butter}`, borderRadius: 9, padding: "9px 16px", cursor: "pointer", fontFamily: mono, fontSize: 12, fontWeight: 600, background: "transparent", color: C.butter }}>Download .txt</button>
              </div>
              ...
              <textarea value={importText} onChange={(e) => { setImportText(e.target.value); setIoMsg(""); }} rows={4} placeholder="Paste a saved CookieBot recipe here…"
                style={{ width: "100%", boxSizing: "border-box", fontFamily: mono, fontSize: 11.5, lineHeight: 1.5, padding: "10px 12px", borderRadius: 10, border: `1.5px solid ${C.line}`, background: C.paperDeep, color: C.ink, outline: "none", resize: "vertical" }} />
              <div style={{ display: "flex", gap: 12, alignItems: "center", flexWrap: "wrap", marginTop: 8 }}>
                <button onClick={loadRecipe} style={{ border: "none", borderRadius: 9, padding: "9px 16px", cursor: "pointer", fontFamily: mono, fontSize: 12, fontWeight: 600, background: C.choc, color: C.onAccent }}>Load recipe</button>
